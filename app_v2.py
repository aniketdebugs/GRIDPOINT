...
def warehouse_count_analysis(
    df,
    max_k,
    rate,
    infrastructure_cost,
    capacities_enabled=True,
    capacity_value=None,
    radius=None
):
    rows = []
    ...
    for k_test in range(1, max_k + 1):
        ...
        if capacities_enabled:
            capacities_test = [float(capacity_value)] * k_test
        else:
            capacities_test = None

        try:
            result_test = optimize_network(
                df,
                k_test,
                rate,
                capacities_test,
            ...
            rows.append({
                "Warehouses": k_test,
                "Delivery Cost": result_test["delivery_cost"],
                "Infrastructure Cost": k_test * infrastructure_cost,
                "Total System Cost": total,
                "Unserved Orders": (
                    df.iloc[result_test["unassigned"]]["daily_orders"].sum()
                    if result_test["unassigned"]
                    else 0
                )
            })
...
st.caption(
    f"Capacity: {vehicle['capacity']} orders | "
    f"Operating cost: ₹{vehicle['cost_per_km']}/km | "
    f"Average speed: {vehicle['speed_kmph']} km/h"
)
...
if input_mode == "Sample Dataset":
    ...
    st.info("Using the included Bengaluru demonstration dataset.")
    ...
    st.markdown("### Add Neighborhoods")

    default_data = pd.DataFrame({
        "neighborhood": ["Koramangala", "Indiranagar"],
        "latitude": [12.9352, 12.9784],
        "longitude": [77.6245, 77.6408],
        "daily_orders": [450, 320]
    })
...
failed_choice = st.selectbox(
    "Simulate warehouse failure",
    ["None"] + [f"Warehouse {i+1}" for i in range(len(df))]
)
failed = []
...
def total_delivery_distance(df, assignment, dmat):
    valid = assignment >= 0
...
optimized_distance = total_delivery_distance(
    df,
    result["assignment"],
...
base_time_hours = optimized_distance / vehicle["speed_kmph"]

traffic_adjusted_time = (
    base_time_hours * traffic_multiplier
)

traffic_adjusted_minutes = traffic_adjusted_time * 60
...
st.markdown("### 🧮 Warehouse Count Trade-off")

tradeoff_df = warehouse_count_analysis(
    df,
    min(5, len(df)),
    rate,
    infra,
    capacities is not None,
    capacities[0] if capacities else None,
    radius
)
...
if not tradeoff_df.empty:
    ...
    st.line_chart(
        tradeoff_df.set_index("Warehouses")["Total System Cost"]
    )
